...
const lifeCycle = () => {
  return {
    ...
    async mount(props) {
      ...
      render(props);
      try {
        props.service.CALL.setCallInfo({
          pd_code: 'CALL_PHONE_SERVICE_CENTRE',
          ad_code: '510100',
        }).then((data) => {
          console.log('初始电话成功', data);
        });
        props.service.CALL.getCallInfo().then(({ callState }) => {
          if (callState === 4) {
            props.service.CALL.addEditUser();
          }
        });
      } catch (err) {
        console.log(err);
