...
class Api {
  constructor(sysCode, secret, API_PREFIX) {
  ...
  doFetch(method, path, data, headers, resolve, reject, isFile = false) {
    ...
    const _path = path.indexOf('http') === 0 ? path : `${this.API_PREFIX || API_PREFIX}${path}`;
...
export function createCrmApi() {
  return new Api(CONFIG.SYS_CODE, CONFIG.SECRET, '/api');
}
export function createResourceApi() {
  return new Api(CONFIG.RESOURCE_SYS_CODE, CONFIG.RESOURCE_SECRET);
}
