...
<script>
import './index.scss';
import EditDemandInfo from '../edit-demand-info/index.vue';
import { CLIENT_DEMAND_INFO } from 'constants/constants';
import { get_requirement_info, del_demand } from 'api/source-details';
...
export default {
  ...
  created() {
    ...
    this.$eventBus.$on('get-business-info', (value) => {
      this.canEdit = value?.bizLibrary === 'PLATFORM' && value?.allocationStatus == 0;
    });
        ...
        .then((res) => {
          if (res.code === 200) {
            ...
            this.$nextTick(() => {
              const containerSize = this.$refs.scrollWarpRef.offsetWidth;
              const navOffset = this.$refs.navWarpRef.offsetWidth;
              this.scrollDisable = navOffset > containerSize ? true : false;
            });
          } else {
            this.$message.warning(res.message);
          }
