  ...
  methods: {
    ...
    getAreaList(item) {
      ...
      get_all_businessArea(params).then((res) => {
        if (res.code === 200) {
          res = res.data || {};
          const curArea = res.cityList?.find((it) => it.code === item?.bizAreaCode);
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid && flag) {
          ...
          let params = {
            ...
            bizAreaName:
              this.areaList.find((item) => item.code === ruleForm.demandArea)?.name || '',
          ...
          params.intentionCode = this.intentionCode;
          this.loading = true;
          path(params)
            .then((res) => {
              if (res.code === 200) {
    ...
    openModal(item, bizId, attributeList, followDemandList) {
      ...
      if (item.requireId) {
        ...
        this.intentionCode = item.intentionCode;
      }
