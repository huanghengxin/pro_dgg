const envs = [
  {
    ...
    branch: 'dev',
    // 镜像名称
    name: 'qds-sp-resource-allocation-0',
    ...
    externalEnvironment: {},
    baseUrl: 'https://dresource-allocation.shupian.cn/',
  ...
  },
  {
    ...
    // 需要额外注入的全局变量，代码中可直接通过 window.__EXTERNAL_ENVIRONMENT__.xx 使用
    externalEnvironment: {},
    baseUrl: 'https://tresource-allocation.shupian.cn/',
    //应用运行环境如:开发环境、测试环境、生产环境
    SP_SERVER_ENV: 'test',
  },
  {
    ...
    // 需要额外注入的全局变量，代码中可直接通过 window.__EXTERNAL_ENVIRONMENT__.xx 使用
    externalEnvironment: {},
    baseUrl: 'https://resource-allocation.shupian.cn/',
    //应用运行环境如:开发环境、测试环境、生产环境
...
const config = {
  ...
  // 子应用名称，作为主应用的匹配规则
  subAppName: 'qds-resource-allcation',
